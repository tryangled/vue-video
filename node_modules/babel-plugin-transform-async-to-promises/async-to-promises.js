...
var helpers_string_1 = require("./helpers-string");
...
var originalNodeMap = new WeakMap();
var skipNodeMap = new WeakMap();
var breakIdentifierMap = new WeakMap();
var isHelperDefinitionMap = new WeakMap();
var helperNameMap = new WeakMap();
var nodeIsAsyncMap = new WeakMap();
;
...
function default_1(_a) {
    ...
    function cloneNode(node) {
        ...
        if (types.isIdentifier(node) || types.isMemberExpression(node)) {
            var helperName = helperNameMap.get(node);
    ...
    function pathsPassTest(matchingNodeTest, referenceOriginalNodes) {
        function visit(path, result, state) {
            if (referenceOriginalNodes) {
                var originalNode = originalNodeMap.get(path.node);
                if (originalNode) {
    ...
    var pathsReturn = pathsReachNodeTypes(["ReturnStatement"], true);
    ...
    function breakIdentifierForPath(path) {
        var result = breakIdentifierMap.get(path.node);
        if (!result) {
    ...
    function insertHelper(programPath, value) {
        var destinationPath = programPath.get("body").find(function (path) { return !isHelperDefinitionMap.get(path.node) && !path.isImportDeclaration(); });
        if (destinationPath.isVariableDeclaration()) {
            var before = destinationPath.get("declarations").filter(function (path) { return isHelperDefinitionMap.get(path.node); });
            var after = destinationPath.get("declarations").filter(function (path) { return !isHelperDefinitionMap.get(path.node); });
            if (types.isVariableDeclaration(value)) {
                ...
                if (before.length === 0) {
                    ...
                    var target = before[before.length - 1];
                ...
                if (before.length === 0) {
                    ...
                    var beforeNode = types.variableDeclaration(destinationPath.node.kind, before.map(function (path) { return path.node; }));
                    isHelperDefinitionMap.set(beforeNode, true);
                    var afterNode = types.variableDeclaration(destinationPath.node.kind, after.map(function (path) { return path.node; }));
            ...
            var oldNode = destinationPath.node;
        ...
        if (result) {
            ...
            if (readConfigKey(state.opts, "externalHelpers")) {
                ...
                if (!helpers) {
                    ...
                    var helperAst = require(isNewBabel ? "@babel/core" : "babylon").parse(helpers_string_1.code, {
                        sourceType: "module",
                        filename: "helpers.js"
                    });
                    if (isNewBabel) {
                ...
                var newPath = insertHelper(file.path, value);
    ...
    function invokeTypeOfExpression(path) {
        if (path.isCallExpression() && types.isIdentifier(path.node.callee)) {
            var helperName = helperNameMap.get(path.node.callee);
            switch (helperName) {
                case "_invoke":
                case "_invokeIgnored":
                case "_catch":
    ...
    function rewriteDefaultArguments(targetPath, pluginState) {
        var statements = [];
        var params = targetPath.get("params");
        var literals = [];
        for (var i = 0; i < params.length; i++) {
            var param = params[i];
            if (param.isAssignmentPattern()) {
                var init = param.get("right");
                if (!isExpressionOfLiterals(init, literals)) {
                    ...
                    var initNode = init.node;
                    param.replaceWith(id);
                    var isMissing = types.binaryExpression("===", id, types.identifier("undefined"));
                    var assignment = types.expressionStatement(types.assignmentExpression("=", id, initNode));
    ...
    var unwrapReturnPromiseVisitor = {
        ReturnStatement: function (path) {
            var argument = path.get("argument");
            if (argument.isCallExpression()) {
                    ...
                    case "resolve":
                        switch (argument.node.arguments.length) {
                            case 0:
                                ...
                                break;
                            case 1:
                                var arg0 = argument.node.arguments[0];
            ...
            FunctionExpression: function (path) {
                if (path.node.async) {
                    ...
                    if (path.node.generator) {
                        ...
                        var parentPath = path.parentPath;
                        var skipReturn = parentPath.isCallExpression() && parentPath.node.callee === path.node && parentPath.parentPath.isExpressionStatement();
                        if (!skipReturn && !pathsReturnOrThrowCurrentNodes(bodyPath).all) {
                            var awaitHelper = inlineHelpers ? promiseResolve() : helperReference(this, path, "_await");
                            ...
                            if (inlineHelpers) {
                                if (skipReturn && parentPath.isCallExpression() && parentPath.node.arguments.length === 0 && !pathsReturn(bodyPath).any) {
                                    ...
                                    path.replaceWith(functionize(this, path.node.params, blockStatement(types.tryStatement(bodyPath.node, types.catchClause(types.identifier("e"), blockStatement([
                                        (skipReturn ? types.expressionStatement : types.returnStatement)(types.callExpression(types.memberExpression(types.identifier("Promise"), types.identifier("reject")), [types.identifier("e")]))
                                    ])))), path));
                                }
                            }
                            else {
